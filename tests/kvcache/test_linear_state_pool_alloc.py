...
def test_snapshot_round_trip_through_the_radix_tree(monkeypatch):
    """Turn 2 of a conversation, end to end on CPU and with no model or kernel: the real
    build_fla_metadata picks each request's snapshot row, the GDN op writes the recurrent + conv
    state into a donatable slot, the tree takes it at a x CHUNK_SIZE boundary, turn 2 diverges
    past that boundary and match_prefix truncates the reuse back to it, and copy_from restores
    ...
    # the tracking path must stay importable without triton, which Metal cannot install
