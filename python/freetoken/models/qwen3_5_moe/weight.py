...
class _DenseReader:
    ...
    def _emit(self, target: str, parts: list[dict[str, torch.Tensor]], stored: QuantScheme | None):
        ...
        if stored is None and served is not None and served.kind is QuantKind.FP8_TENSOR:
            # --dense-quant-override fp8: the checkpoint stored this module bf16, so quantize
            # the fused tensor the layer will hold; stored is None, so out is the one weight.
