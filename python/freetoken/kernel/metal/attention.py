...
# A whole decode batch in one launch, like MLX's sdpa_vector: a threadgroup per (query head,
# request), BN SIMD-groups striding its KV positions, the 32 lanes splitting the head dim.
_DECODE_SRC = r"""
...
def _decode_lib(head_dim: int, q_heads: int, group: int, dtype: torch.dtype, bn: int,
    ...
    src = _DECODE_SRC.format(
        d=head_dim, hq=q_heads, group=group, bn=bn, ept=-(-head_dim // 32), qpt=1
    )
...
def paged_decode_attention_mps(
    ...
    bn = max(1, min(8, _TG_BYTES // (4 * head_dim + 8)))
    ...
    lib = _decode_lib(head_dim, q_heads, q_heads // kv_heads, q.dtype, bn, gate is not None)
    lib.paged_decode_attention(
        *args, threads=(32 * bn, q_heads, bs), group_size=(32 * bn, 1, 1),
    )
    return out


# FlashAttention-2 prefill: a threadgroup per (query tile, head) walks KV in C-wide tiles with an
# online softmax; paged K/V stages through threadgroup memory, split across SIMD-groups by column.
_PREFILL_SRC = r"""
#include <metal_stdlib>
...
# (SIMD-groups, KV tile C, query tile BQ), best measured first: C is 8 score columns per
# SIMD-group and at most 32, BQ/NSG is whole softmax passes, BQ 32 spills at head_dim 256.
...
def paged_attention_mps(
    ...
    for i in range(len(q_ptr) - 1):
        ...
        k = k_cache.index_select(0, slots).transpose(0, 1).unsqueeze(0)
        v = v_cache.index_select(0, slots).transpose(0, 1).unsqueeze(0)
