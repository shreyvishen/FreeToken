...
// out[m, n] = scale[n] * sum_k x[m, k] * e4m3(w[n, k]). The activation row is read straight from
// device memory rather than staged in threadgroup memory: every lane consumes a different slice
// of it, so staging broadcasts nothing and only spends the threadgroup memory that caps
...
#if NORM
  // Prologue: residual add + RMSNorm of this token's row, so the norm is not a launch of its
  // own. Threadgroup 0 alone writes the new residual, out of place, so no threadgroup reads a
  // row another is writing.
