"""Dense fp32 / fp16 / bf16 GEMV and a SwiGLU-fused gate/up GEMV, as single Metal launches."""
...
# (SIMD-groups per threadgroup, output rows per SIMD-group), as nvfp4.py's _TILES: the first
# entry that divides the row count and still fills _MIN_TGS threadgroups wins.
_TILES = ((16, 2), (16, 1), (8, 1), (4, 1), (2, 1), (1, 1))
_MIN_TGS = 16

# MAX_GEMV_ROWS is fp8.py's, re-exported for MetalLinearKernel.
...
def _gemv(x: torch.Tensor, w: torch.Tensor, n: int, swiglu: bool, norm=None):
    m, k = x.shape
    nsg, nr0 = pick_tile(_TILES, n, _MIN_TGS)
    out = torch.empty((m, n), dtype=x.dtype, device=x.device)
    rows = nsg * nr0
    args = [out, x, w]
    xnorm = None
    if norm is not None:
        # The caller gates on can_run_norm (MetalLinearKernel.apply_fused).
