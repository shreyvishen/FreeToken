...
class OffloadMoELayer(MoELayer):
    ...
    def predict_from_router(self, router_logits: torch.Tensor) -> None:
        ...
        pr = getattr(self.offload_cache, "predict_experts", None)
        if pr is not None:
            pr(self.layer_id, torch.topk(router_logits, self.top_k, dim=-1).indices)

    def issue_from_prediction(self) -> None:
        """Start the reads the previous layer's :meth:`predict_from_router` named."""
        iss = getattr(self.offload_cache, "issue_prediction", None)
        if iss is not None:
            iss(self.layer_id)
