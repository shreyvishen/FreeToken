...
class TritonAttentionBackend(BaseAttnBackend):
    ...
    def __init__(self, config: ModelConfig):
        ...
        self._indices_buf: torch.Tensor | None = None  # MPS: see _page_indices
        self.max_graph_bs = 0
    ...
    def _page_indices(self, page_table, reqs, seqlens_k: List[int]) -> torch.Tensor:
        """Each request's live page-table slots, concatenated: the KV slot list every kernel
        reads through ``indptr``."""
        if page_table.device.type != "mps":
            return torch.cat([page_table[req.table_idx, : req.device_len] for req in reqs])
        total = sum(seqlens_k)
        buf = self._indices_buf
        if buf is None or buf.numel() < total:
            buf = torch.empty(
                1 << max(total - 1, 0).bit_length(), dtype=page_table.dtype,
                device=page_table.device,
            )
            self._indices_buf = buf
        indices = buf[:total]
        offset = 0
    ...
    def prepare_metadata(self, batch: Batch) -> None:
        ...
        indices = self._page_indices(page_table, reqs, seqlens_k)
        swa_indices = None
